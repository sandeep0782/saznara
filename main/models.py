...
class SKU(models.Model):
    ...
    blouse = models.CharField(
        max_length=50, choices=BLOUSE_CHOICES, null=True, blank=True
    )
    border = models.CharField(
        max_length=50, choices=BORDER_CHOICES, null=True, blank=True
    )
    print_or_pattern_type = models.CharField(
        max_length=50, choices=PRINT_PATTERN_TYPE_CHOICES, null=True, blank=True
    )
    saree_fabric = models.CharField(
        max_length=50, choices=SAREE_FABRIC_CHOICES, null=True, blank=True
    )
    transparency = models.CharField(
        max_length=50, choices=TRANSPARENCY_CHOICES, null=True, blank=True
    )
    type = models.CharField(max_length=50, choices=TYPE_CHOICES, null=True, blank=True)
    blouse_length = models.CharField(
        max_length=50, choices=BLOUSE_LENGTH_SIZE_CHOICES, null=True, blank=True
    )
    saree_length = models.CharField(
        max_length=50, choices=SAREE_LENGTH_SIZE_CHOICES, null=True, blank=True
    )
    blouse_color = models.CharField(
        max_length=50, choices=BLOUSE_COLOR_CHOICES, null=True, blank=True
    )
    blouse_fabric = models.CharField(
        max_length=50, choices=BLOUSE_FABRIC_CHOICES, null=True, blank=True
    )
    blouse_pattern = models.CharField(
        max_length=50, choices=BLOUSE_PATTERN_CHOICES, null=True, blank=True
    )
    border_width = models.CharField(
        max_length=50, choices=BORDER_WIDTH_CHOICES, null=True, blank=True
    )
    loom_type = models.CharField(
        max_length=50, choices=LOOM_TYPE_CHOICES, null=True, blank=True
    )
    occasion = models.CharField(
        max_length=50, choices=OCCASION_CHOICES, null=True, blank=True
    )
    ornamentation = models.CharField(
        max_length=50, choices=ORNAMENTATION_CHOICES, null=True, blank=True
    )
    pallu_details = models.CharField(
        max_length=50, choices=PALLU_DETAILS_CHOICES, null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        # Generate SKU
        # ===========================
        generated_sku = f"{self.ref_no}-{self.color}"
        ...
        # Generate VAN
        # ===========================
        self.van = f"{self.ref_no}-{self.color}"

        # ===========================
        # Generate EAN
        # ===========================
        self.ean = f"{self.brand.abb}-{self.ref_no}-{self.color}"
        ...
        if not self.barcode_image or not old_instance or old_instance.ean != self.ean:
...
class VMS(models.Model):
    ...
    )  # e.g., 'purchase', 'sale', 'return'
    video_file = models.FileField(upload_to="videos/")
    video_type = models.CharField(
        max_length=20, choices=VIDEO_TYPE_CHOICES, default="forward"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class MarketplaceMapping(models.Model):
    ...
    ATTRIBUTE_CHOICES = (
        ...
        ("TECHNIQUE", "Technique"),
        ("OCCASION", "Occasion"),
        ("SAREE_FABRIC", "Saree Fabric"),
        ("BLOUSE_FABRIC", "Blouse Fabric"),
        ("BLOUSE", "Blouse"),
        ("PATTERN", "Pattern"),
        ("PRINT_OR_PATTERN_TYPE", "Print Or Pattern Type"),
        ("ORNAMENTATION", "Ornamentation"),
        ("BORDER", "Border"),
        ("BLOUSE_PATTERN", "Blouse Pattern"),
        ("PALLU_DETAILS", "Pallu Details"),
        ("BORDER_WIDTH", "Border Width"),
    )

    marketplace = models.CharField(
        max_length=50, choices=MARKETPLACE_CHOICES, db_index=True
    )
    attribute = models.CharField(
        max_length=100, choices=ATTRIBUTE_CHOICES, db_index=True
    )
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=["marketplace", "attribute", "source_value"]),)

...
def get_snapdeal_color(color_name):
    if not color_name:
        return None
    return SNAPDEAL_COLOR_MAPPING.get(color_name.strip())
...
def get_snapdeal_blouse_color(blouse_color_name):
    if not blouse_color_name:
        return None
    return SNAPDEAL_BLOUSE_COLOR_MAPPING.get(blouse_color_name.strip())
...
def get_snapdeal_technique(type):
    if not type:
        return None
    return SNAPDEAL_TECHNIQUE_MAPPING.get(type.strip())
...
def get_snapdeal_saree_fabric(saree_fabric):
    if not saree_fabric:
        return None
    return SNAPDEAL_SAREE_FABRIC_MAPPING.get(saree_fabric.strip())
...
def get_snapdeal_blouse_fabric(blouse_fabric):
    if not blouse_fabric:
        return None
    return SNAPDEAL_BLOUSE_FABRIC_MAPPING.get(blouse_fabric.strip())


# SNAPDEAL BLOUSE
SNAPDEAL_BLOUSE_MAPPING = {"separate_blouse_piece": "With Unstitched Blouse Piece"}


def get_snapdeal_blouse(blouse):
    if not blouse:
        return None
    return SNAPDEAL_BLOUSE_MAPPING.get(blouse.strip())
...
def get_snapdeal_pattern(pattern):
    if not pattern:
        return None
    return SNAPDEAL_PATTERN_MAPPING.get(pattern.strip())
...
def get_snapdeal_blouse_pattern(blouse_pattern):
    if not blouse_pattern:
        return None
    return SNAPDEAL_BLOUSE_PATTERN_MAPPING.get(blouse_pattern.strip())
...
def get_snapdeal_print_or_pattern_type(print_or_pattern_type):
    if not print_or_pattern_type:
        return None
    return SNAPDEAL_PRINT_OR_PATTERN_TYPE_MAPPING.get(print_or_pattern_type.strip())
...
def get_snapdeal_ornamentation(ornamentation):
    if not ornamentation:
        return None
    return SNAPDEAL_ORNAMENTATION_MAPPING.get(ornamentation.strip())
...
def get_snapdeal_border(border):
    if not border:
        return None
    return SNAPDEAL_BORDER_MAPPING.get(border.strip())
